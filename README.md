...
## Core Requirements
The core requirements for this assignment can be found in the Canvas assignment description. Please refer to that for the specific tasks you need to complete. The main focus of this assignment is to implement a Vue application that supports the RUD part of CRUD: display a list of items, view details of a single item, and edit an item. You can leave out the Create/New Item form for now, as it won't be very meaningful without a real backend to handle file uploads. If you don't have file uploads in your app, you can include the Create form if you like, but it's not required for this assignment.
...
### Deploy your app
- Deploy to Render (recommended) or DigitalOcean.
...
- Remember that `npm run build` runs on Render, so make sure that your build command is set to `npm run build` and your publish directory is set to `dist` in your Render settings.
